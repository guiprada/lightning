...
mv lightning_interpreter/bin/Release/net10.0/linux-x64/* linux_builds/
...
REF_DIR=$(find /usr/lib/dotnet/packs/Microsoft.NETCore.App.Ref -name "System.Runtime.dll" -path "*/net10.0/*" 2>/dev/null | head -1 | xargs -r dirname)
if [ -n "$REF_DIR" ]; then
