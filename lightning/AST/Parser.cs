    ...
    public class Parser
    {
        struct FunctionStruct
        {
            ...
            public FunctionStruct(
                List<ParameterInfo> p_parameters,
        ...
        List<ParameterInfo> Parameters()
        {
            ...
            do
            {
                ...
                {
                    identifier = (TokenString)Consume(
                        TokenType.IDENTIFIER,
                        "Expected 'identifier' as 'function parameter'.",
                        true

                    );
                    ParameterInfo new_parameter = new ParameterInfo(identifier.value, is_mut);
                ...
                if (Check(TokenType.COMMA))
                {
                    ...
                    ElideMany(TokenType.NEW_LINE);
                    go_on = Check(TokenType.IDENTIFIER) || Check2(TokenType.IDENTIFIER);
                    ElideMany(TokenType.NEW_LINE);
                }
                else
                {
        ...
        Token Consume(TokenType p_type, string p_msg, bool p_error)
        {
            ...
            {
                string str = p_msg + " Found: " + Peek();
                if (p_error == true)
                    Error(str, Peek().PositionData);
                else
                    Warning(str, Peek().PositionData);
